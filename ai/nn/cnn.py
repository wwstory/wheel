...
def conv2d(input, weight, bias=None, stride=1, padding=0, dilation=1):
    ...
    Args:
        ...
        dilation (int|np.ndarray, optional): 扩展视野域. Defaults to 1.
    ...
    output_size = cal_output_size(img.shape[1:], dilation_size, stride)
...
if __name__ == "__main__":
    ...
    img = np.random.randn(3, 10, 20).clip(0, 1)
    input_channel = 3
    output_channel = 4
    kernel = np.random.randn(output_channel, input_channel, 3, 3)   # weight
